...
namespace Norwix
{
    public partial class uc_Nowix : UserControl
    {
            ...
            this.Invoke(new Action(() =>
            {
                switch (e)
                {
                    ...
                    case Enum_ConnectionEventClient.BUTTON_STATE:                       
                        if ((int)obj >5) break;
                        if (!ena && (e_BUTTON_STATE)((int)obj) == e_BUTTON_STATE.CONFIGURING_PRINTER) return;
        ...
        public void AddData(List<string> data)
        {
            DATA.AddRange(data);
            Total = DATA.Count - 1;
            Invoke(new Action(() => { UpdateCounter(); SendData(); }));
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            {
                if (MAYIN == e_BUTTON_STATE.PRINTING_STOPED || MAYIN == e_BUTTON_STATE.STOPPING_OR_ERROR || MAYIN == e_BUTTON_STATE.STOPPING_PRINTED)
                {
                    if (DATA.Count == 0)
                    {
        ...
        {
            if(DATA.Count==0) { MessageBox.Show("Chưa có dữ liệu in"); return; }
            btnPrint.Enabled = false;
            if (sendPrinter.Connected && recivePrinter.Connected)
            {
